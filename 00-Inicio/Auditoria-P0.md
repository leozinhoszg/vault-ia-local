...
| Lockfile e gate | Lock Windows/Python 3.11 regenerado com uv 0.12.8 e hashes: 43 pins, sha256 `f5256db8…`; requirements diretos reconciliados contra o lock; pypdf mínimo 6.15.0; SCA fail-closed; Actions por SHA; Dependabot mensal. | Gate local do commit `c4ecf97`: validador `--strict` exit 0 (0 erros/avisos), índice `--check` exit 0, `check_tco.py` PASS, checksum OK, selftest OK e pip-audit 2.10.1 sem vulnerabilidades conhecidas. A execução `33524666554` continua evidência histórica do pipeline anterior; o CI remoto no SHA atual ainda precisa ser observado no PR. |
